    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        ...
        if(key.equals(KEY_KIDS_MODE_SWITCH)){
            if(sharedPreferences.getBoolean(KEY_KIDS_MODE_SWITCH,false)){
                ...
                Toast.makeText(this, "Score ranking disabled: Cannot rank when kids mode is enabled.\"", Toast.LENGTH_SHORT).show();
             }else{
                kidsModeNotDisabled = false;
                if(!timerDisabled && !typeDisabled) {
                     Toast.makeText(this, "Score ranking has been re-enabled.", Toast.LENGTH_SHORT).show();
        ...
        if(key.equals(KEY_ADDITION_ADVANCED)
                ...
                || key.equals(KEY_CUBE)){
            if(!sharedPreferences.getBoolean(KEY_ADDITION_ADVANCED,true)
                    || !sharedPreferences.getBoolean(KEY_SUBTRACTION_ADVANCED,true)
                    || !sharedPreferences.getBoolean(KEY_ADDITION_X_MULTIPLICATION,true)
                    || !sharedPreferences.getBoolean(KEY_SUBTRACTION_X_MULTIPLICATION,true)
                    || !sharedPreferences.getBoolean(KEY_ADDITION_BY_DIVISION,false)
                    || !sharedPreferences.getBoolean(KEY_SUBTRACTION_BY_DIVISION,false)
                    || !sharedPreferences.getBoolean(KEY_SQUARE,false)
                    || !sharedPreferences.getBoolean(KEY_SQUARE_ROOT,false)
